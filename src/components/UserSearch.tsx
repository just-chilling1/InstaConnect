...
type UserSearchProps = {
  ...
  onNavigate?: () => void;
  /** Start a DM instead of navigating to profile (messages page). */
  onSelectUser?: (userId: string) => void;
  className?: string;
};

export default function UserSearch({ onNavigate, onSelectUser, className = "" }: UserSearchProps) {
  ...
  function goToProfile(username: string, targetUserId?: string) {
    setOpen(false);
    setQuery("");
    if (onSelectUser && targetUserId) {
        ...
        <div className="absolute left-0 right-0 mt-2 rounded-lg border border-border bg-surface shadow-lg z-40 overflow-hidden max-h-80 overflow-y-auto">
          ...
          {!isLoading &&
            results.map((profile) => (
              <button
                key={profile.id}
                onClick={() => goToProfile(profile.username, profile.user?.id)}
