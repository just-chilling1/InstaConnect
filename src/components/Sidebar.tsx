...
import { useUnreadBadges } from "@/lib/useUnreadBadges";
...
export default function Sidebar() {
  ...
  // Unread badges — shared hook (one subscription).
  const { unreadNotifications, unreadMessages } = useUnreadBadges(user?.id);
  ...
  const navItems = [
    ...
    { href: "/explore", label: "Explore", Icon: IconCompass, exact: true },
    { href: "/search", label: "Search", Icon: IconSearch, exact: true },
    ...
    { href: "/saved", label: "Saved", Icon: IconBookmark, exact: true },
    { href: profileHref, label: "Profile", Icon: IconUserCircle, exact: false },
    {
      ...
      exact: false,
      badge: formatBadgeCount(unreadMessages),
    },
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <BrandMark className="w-8 h-8 text-accent flex-shrink-0" />
          ...
          return (
            ...
            >
              <span className="relative flex-shrink-0 flex items-center justify-center">
                ...
                {item.badge && (
                  <span className="absolute -right-1.5 -top-1.5 min-w-[16px] h-4 px-1 rounded-full bg-accent text-accent-text text-[10px] font-mono font-medium flex items-center justify-center">
                    {item.badge}
                  </span>
                )}
              </span>
