...
export default function FollowButton({ targetUserId, currentUserId }: FollowButtonProps) {
  ...
  // Query by the follower side only and match the target client-side.
  // Combining two dot-path link filters ("follower.id" AND "following.id")
  // in a single `where` has proven unreliable at runtime, which made the
  // button think the user was never following anyone (so unfollow was
  // unreachable). A single link filter is the query shape used everywhere
  // else in the app and is known to work.
  const followQuery = {
    follows: {
  ...
  const matches = useMemo(
    () => (data?.follows ?? []).filter((f) => f.following?.id === targetUserId),
    [data?.follows, targetUserId]
  );
  ...
  async function toggleFollow() {
    ...
    try {
      if (wasFollowing) {
        ...
        const target = fresh.data.follows.filter((f) => f.following?.id === targetUserId);
        if (target.length > 0) {
          await db.transact(target.map((m) => db.tx.follows[m.id].delete()));
        }
        return;
      }
      ...
      try {
        const recipient = await db.queryOnce({
          profiles: { $: { where: { "user.id": targetUserId } } },
        });
