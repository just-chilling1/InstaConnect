...
import { openConversation as openOrCreateConversation } from "@/lib/openConversation";
...
function MessagesContent() {
  ...
  useEffect(() => {
    ...
    unreadMessages.forEach((m) => markedReadRef.current.add(m.id));
  ...
  }, [messages.length, user?.id, activeConversationId]);
    ...
    try {
      ...
      if (otherId && otherId !== user.id) {
        const recipient = await db.queryOnce({
          profiles: { $: { where: { "user.id": otherId } } },
        });
  ...
  async function startConversationWith(targetUserId: string) {
    if (!user || newMessageBusy) return;
    setNewMessageBusy(true);
    try {
      const convId = await openOrCreateConversation(user.id, targetUserId);
      setNewMessageOpen(false);
      if (convId) router.push(`/messages?c=${convId}`);
    } catch (err) {
      alert(err instanceof Error ? err.message : "Couldn't start conversation.");
    } finally {
  ...
  async function saveMessageEdit(messageId: string) {
    ...
    if (!trimmed) return;
    await db.transact([
      db.tx.messages[messageId].update({ content: trimmed, editedAt: Date.now() }),
    ]);
  ...
  async function deleteMessage(messageId: string) {
    ...
    await db.transact([db.tx.messages[messageId].delete()]);
  }
      ...
      >
        ...
        <div className="px-4 pt-5 pb-3 border-b border-border flex-shrink-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={() => setNewMessageOpen((v) => !v)}
              className="flex items-center justify-center w-8 h-8 rounded-lg text-text-muted hover:bg-surface-2 hover:text-text transition-colors"
          ...
          {newMessageOpen && (
            <div className="mb-3">
              <UserSearch
                onNavigate={() => setNewMessageOpen(false)}
                onSelectUser={startConversationWith}
              />
              {newMessageBusy && (
                <p className="text-xs text-text-faint mt-2">Opening conversation...</p>
              )}
            </div>
          )}
          {/* Search */}
                        ...
                        {editingMessageId === msg.id ? (
                          <div className="flex flex-col gap-2 w-full">
                            <textarea
                              ...
                              className="w-full bg-ink border border-border rounded-lg px-3 py-2 text-sm text-text outline-none focus:border-accent resize-none"
                              rows={2}
                            />
                          ...
                          <>
                            <div
                              className={`group/msg relative px-3.5 py-2 text-sm leading-relaxed whitespace-pre-wrap break-words
                            ${isMine
                            ...
                            >
                              {msg.content}
                              {isMine && (
                                <span className="absolute -top-2 -right-1 hidden group-hover/msg:flex gap-0.5">
                                  <button
                                    onClick={() => {
                                      setEditingMessageId(msg.id);
                                      setEditMessageText(msg.content);
                                    }}
                                    className="p-1 rounded-full bg-surface border border-border text-text-muted hover:text-text"
                                  ...
                                  <button
                                    ...
                                    className="p-1 rounded-full bg-surface border border-border text-text-muted hover:text-negative"
