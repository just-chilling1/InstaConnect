  ...
  async function onSubmit(values: CreatePostValues) {
    ...
    try {
      ...
      if (file) {
        ...
        const uploadFile =
          mediaType === "image" ? await compressImage(file) : file;
        // eslint-disable-next-line react-hooks/purity -- see note above onSubmit
        const path = `posts/${user.id}/${Date.now()}-${uploadFile.name}`;
        const { data: uploaded } = await db.storage.uploadFile(path, uploadFile, {
